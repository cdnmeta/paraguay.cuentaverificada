...
import { aprobarCuenta, generarTokenSolicitudById, rechazarSolicitud } from "@/apis/verificacionCuenta.api";
...
export default function ListadoSolicitudes({
  ...
  const { aprobarSolicitudes = false, rechazarSolicitudes = false, generarTokenUsuario = false } =
    opcionesLista;
...
const VistaTokenGenerado = ({ token, loading, documento }) => {
  ...
  if (!token) return <p className="text-red-500">No se pudo generar el token.</p>;

  const url = generarUrlInicializacionDeCredenciales({ token, cedula: documento });
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative">
        <p className="break-all rounded-md bg-muted p-2 pr-12">
          {url}
        </p>
        <Button
          size="sm"
...
function TokenDialog({ solicitudId }) {
  ...
  return <VistaTokenGenerado token={token} loading={loading} documento={documento} />;
}
      ...
      return (
        <div className="flex gap-1">
          ...
          {rechazarSolicitudes && (
            ...
            {
              generarTokenUsuario && solicitud.id_estado == 3 && (
                <Tooltip>
                  <TooltipTrigger>
                    <Button
                      size={"sm"}
  ...
  const FormRechazoSolicitud = ({ solicitud }) => {
    const schemaRechazo = z.object({
      motivo_rechazo: z.string().min(2).max(100).nonempty("Este campo es obligatorio"),
    ...
    return (
      <form onSubmit={formRechazo.handleSubmit(handleRechazarSolicitud)} className="space-y-4">
      <div>
        <p className="font-semibold">
      ...
      <div className="flex flex-col gap-2">
        ...
        <Checkbox
          ...
          onCheckedChange={val => formRechazo.setValue("confirmar_rechazo", !!val)}
        />
        ...
        <Button
          ...
          variant="destructive"
        >
          {formRechazo.formState.isSubmitting ? "Rechazando..." : "Rechazar Solicitud"}
        </Button>
        </div>
      </div>
      </form>

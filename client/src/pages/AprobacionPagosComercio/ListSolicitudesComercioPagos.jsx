...
import { getComerciosAprovacionPagos, getComerciosForUser } from "@/apis/comercios.api";
...
export default function ListSolicitudesComercioPagos() {
  ...
  const loadComercios = async () => {
    ...
    setcomercioConfirmarpago(comercios.data);
      ...
      toast.error("Error al cargar los comercios para confirmar pagos." + error?.message || "Error desconocido al cargar comercios");
    }
  };
  ...
  const columnasComercio = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <span>{new Date(comercio.fecha_solicitud_verificacion).toLocaleDateString()}</span>;
      },
    },
    {
      header:"Vendedor",
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const comercio = row.original;
        return <BadgeEstadoComercio estado={comercio?.id_estado_comercio_actual} />;
      },
    },

    {
      header: "Opciones",
      cell: ({ row }) => <OpcionesComercio reloadComercios={handleReloadComercios} comercio={row.original} />,
    },
  ];
